...
\begin{sol}
    \begin{itemize}
        ...
        \textbf{Method II:} The two side of the inequality in \ref{P-4-Method-II-condition} reach equal only $\frac{f_{Z0}(z)}{f_Z(z)}=1$. Therefore, $h(Z)=1+\log_2A$ if and only if $Z$ is uniformly distributed between $-A$ and $+A$.
    \end{itemize}
\end{sol}
\end{document}